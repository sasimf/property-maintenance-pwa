...
function PostJob() {
  ...
  const [formData, setFormData] = useState({
    ...
    preferredDate: '', address: user?.address || '', postcode: user?.postcode || '',
    tenantName: '', tenantPhone: '', files: null
  });
  ...
  const handleSubmit = async e => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (key === 'files' && value) Array.from(value).forEach(file => data.append('files', file));
